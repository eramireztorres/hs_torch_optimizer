...
dynamic_regression_file_path = os.path.join(
    os.path.dirname(__file__), "dynamic_regression_model.py"
)
dynamic_image_file_path = os.path.join(
    os.path.dirname(__file__), "dynamic_image_model.py"
)
dynamic_image_regression_file_path = os.path.join(
    os.path.dirname(__file__), "dynamic_image_regression_model.py"
)
...
class DynamicModelUpdater:
    ...
    def update_model_code(self, new_model_code):
        ...
        try:
            if (
                "class LabelSmoothingCrossEntropy(nn.Module):" in new_model_code
                and "loss = loss * class_weights[target]" in new_model_code
            ):
                new_model_code = new_model_code.replace(
                    "loss = loss * class_weights[target]",
                    "loss = loss * class_weights.to(target.device)[target]",
                )
                logging.info(
                    "Applied patch for class_weights device mismatch in LabelSmoothingCrossEntropy."
                )
        ...
        try:
            ...
            if not os.path.exists(self.dynamic_file_path):
                raise FileNotFoundError(
                    f"Dynamic model file '{self.dynamic_file_path}' not found."
                )
            ...
            spec = importlib.util.spec_from_file_location(
                module_name, self.dynamic_file_path
            )
            ...
            if not hasattr(dynamic_module, "load_model"):
                raise AttributeError(
                    f"'load_model()' function not found in {self.dynamic_file_path}"
                )
