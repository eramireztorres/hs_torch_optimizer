...
class TestDataLoader:
    ...
    def test_load_csv_directory_pre_split(self, temp_dir):
        ...
        pd.DataFrame(np.random.randn(80, 10)).to_csv(
            csv_dir / "X_train.csv", index=False
        )
        pd.DataFrame(np.random.randint(0, 2, 80)).to_csv(
            csv_dir / "y_train.csv", index=False
        )
        pd.DataFrame(np.random.randn(20, 10)).to_csv(
            csv_dir / "X_test.csv", index=False
        )
        pd.DataFrame(np.random.randint(0, 2, 20)).to_csv(
            csv_dir / "y_test.csv", index=False
        )
    ...
    def test_load_csv_directory_unsplit(self, temp_dir):
        ...
        pd.DataFrame(np.random.randint(0, 2, 100)).to_csv(
            csv_dir / "y.csv", index=False
        )
